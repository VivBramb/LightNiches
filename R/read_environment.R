...
## light attenuation curve at depth
...
deployment <- read.csv("D:/Dropbox/My Dropbox/NC-RR_environment_data/environment/deployment_scheme_checked.csv", h=T)
...
# filter units used at the RR scale, as in this deployment scheme pairs used in other designs are also stored
depl_scheme18 <- filter(deployment_unit18, RR.COL == "RR")
str(depl_scheme18) 

# read hobos looping in all the folders 
## PS data organization changed each year, so code to produce output differs
...
# H51 was set to wrong day - didn't start.

#### filter out hobo handling  errors ####
## VB set the initial date/time of recording wrong when launching them. 
## they are all from 2017 so VB is not surprised because she was a newbie...
## take them out
...
light <- store_h_all
str(light)
light$mol_light <- 0.0185*light$light # translation coefficient from Tilman

# leaving useful bits here
# light12h <- light %>% 
...
length(unique(light$integral_lux[light$integral_lux>6000000]))
unique(light$hoboID[light$integral_lux>6000000])
unique(light$folder[light$integral_lux>6000000])
...
# check differences in rolling mean breath
...
# at time stamps the observations wasn't made
